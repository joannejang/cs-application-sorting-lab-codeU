...
public class ListSorter<T> {
	...
	private List<T> merge(List<T> first, List<T> second, Comparator<T> comparator) {
		...
		for (int i = 0; i < total; i++) {
			List<T> winner = compareTwo(first, second, comparator);
			result.add(winner.remove(0));
		}
		return result;
	}

	private List<T> compareTwo(List<T> first, List<T> second, Comparator<T> comparator) {
		if (first.size() == 0) {
			return second;
		}
		if (second.size() == 0) {
	...
	public List<T> topK(int k, List<T> list, Comparator<T> comparator) {
		...
		for (T element: list) {
			...
			int compared = comparator.compare(element, heap.peek());
			if (compared > 0) {
				heap.poll();
				heap.offer(element);
			}
